def read_view_files
	...
	for filename in files
		if filename.to_s.end_with?(".haml") 	
			...
			filename = filename.to_s.gsub('html.haml', 'html.erb').gsub('haml', 'html.erb')
			puts "filename: #{filename}"
		end
	end
	#run_command("ruby insertID.rb #{$view_dir}")
	root, files, dirs = os_walk($view_dir)
	for filename in files
	...
	for filename in files
		if filename.to_s.end_with?("html.erb") 
			...
			#new_file_name = new_file_name.gsub('html.erb','rb')
			#TODO: full path
			puts "new_file_name #{new_file_name}"
			...
			begin
				...
				$cur_action = view_file
				traverse_ast(ast, 0)	
				puts "read the: #{view_file.file_path}"
			rescue
			end
		end
	end
