...
def traverse_ast(astnode, level)
	if astnode.class.to_s == "YARD::Parser::Ruby::MethodDefinitionNode"
		...
		if $cur_action.controller.file_name.include?'fulcrum'
			if $cur_action.name == 'index' and $cur_action.controller.name == 'StoriesController'
				fnode = YARD::Parser::Ruby::RubyParser.parse("render partial: 'stories/story'").root.children[0]
				rnder = Render_stmt.new($cur_action, fnode)
			end
		end
		...
		#puts "method call #{astnode.children[0].source}"
		...
		#puts "NEXT LEVEL #{astnode.source}"
		astnode.children.each do |child|
			traverse_ast(child, level+1)
		end
	else
		puts "else #{astnode.source}"
	end
end
