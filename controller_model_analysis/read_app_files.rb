...
def read_each_class(class_name, class_node, filename)
		...
		puts "read_each_class #{class_name} #{filename}"
...
def read_key_words
	...
	if $app_dir and $app_dir.include?'fulcrum'
		$key_words['errors'] = 'SELECT'
		$key_words['full_messages'] = 'SELECT'
	end
