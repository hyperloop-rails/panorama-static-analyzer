...
class INode
	...
	attr_accessor :is_unbounded_loop
	def setCardLimited
		@card_limited = true
	end
	def getClosureStack
	...
	def getUnboundedNonViewClosureStack
		@unbounded_stack = Array.new
		@closure_stack.each do |c|
			if isNonClosureInstr(c.getInstr)
			elsif !@is_unbounded_loop	 
			else
				@unbounded_stack.push(c)
			end 
		end	
		return @unbounded_stack
	end
	...
	def addDataflowEdge(edge)
		...
		begin
			puts "ADD EDGE #{edge.getFromNode.getInstr.toString} to #{edge.getToNode.getInstr.toString}"
		rescue
		end
	end
	...
	#puts "GET FIELD instr #{node.getIndex}, dep_on_attr_assign = #{dep_on_attr_assign} #{to_ins.getClassName} #{to_ins.getCallerType}"
		...
		if instr.getClassName == instr.getCallerType and $class_map[instr.getClassName] and $class_map[instr.getClassName].member_defs[instr.getFuncname]
			$class_map[instr.getClassName].member_defs[instr.getFuncname].each do |p|
				...
				puts "3: from : #{p.getInstr.toString} to #{to_ins.toString}"
...
def add_dataflow_edge(node)
	...
	to_ins.getDeps.each do |dep|
		...
		if dep.getInstrHandler.getINode != nil
			...
			if from_node.getInstr.hasClosure?
				from_node.getInstr.getClosure.getClosureDefs(dep.getVname).each do |t|
					...
					if actual_from_ins != node.getInstr	
						...
						puts "0: from #{actual_from_ins.toString} to #{node.getInstr.toString}"
			...
			from_ins = from_node.getInstr
			#if (dep.getVname == "%self") and (dep.getBlock == 2) and (dep.getInstr == 0) and from_node.getInstr.getClassName.include?'Controller'
			
			if (dep.getVname == "%self") and from_ins.instance_of?Instruction and from_node.getInstr.getClassName.include?'Controller' 
				puts "1 %self #{from_ins.class.name} #{from_node.getInstr.toString} ===> #{to_ins.toString}"
			...
			elsif dep.getVname == "%self" and to_ins.is_a?Call_instr and from_ins.is_a?Call_instr and to_ins.getFuncname != from_node.getInstr.getFuncname
			...
			#TODO: Here we assume all non-defined functions in view are read only...
				...
				$dataflow_edges[edge_name].push(edge)
				puts "1: from #{dep.getInstrHandler.toString} to #{node.getInstr.toString}"
			end
		end
	end
end
