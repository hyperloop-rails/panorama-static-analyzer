	...
	for filename in files
			if filename.to_s.end_with?(".log") and filename.to_s.include?("schema.log") == false
				...
				if $const and !$const.include?class_name
					puts "#{$const}"
					puts "#{class_name} #{filename.to_s} is not used"
					next
				end
...
def find_method(dataflow_file_name, method_name, is_self = false)
	class_name = dataflow_filename_match_function(dataflow_file_name, method_name)
	return $class_map[class_name].getMethod(method_name, is_self)
end
...
def handle_single_dataflow_file(item, class_name)
		...
		file.each_line do |line|
			if line.include?("SET IRMethod")
				...
				m = find_method(item, func_name, !$cur_cfg.getInstanceMethod )
				...
				if m == nil
					...
					puts "METHOD FOUND #{item} #{func_name} #{m.is_self}" 
					m.setCFG($cur_cfg)
				end
				...
				if attrs.include?('(inherit')
					index_inh = attrs.index('(inherit')
					lower = attrs[index_inh + 1]
					attrs[index_inh] = '(inherit ' + lower
					attrs.delete_at(index_inh + 1)
				end
				...
				if i!= -1
					...
					if attr[-1,1] == ':'
						if attr.include?("outgoing")
								...
								if index > i
									...
									if single_attr.include?('"')
										...
										if single_attr.include?("inherit ") and $cur_bb.getInstr[-1].is_a?Const_instr
												...
												puts "inherit cur_instr : #{upper_name}::#{cur_name}"
										else
							...
							if cur_instr.instance_of?GetField_instr
								ori_file = item.gsub('.log','.rb').gsub('dataflow','')
								puts "item file name : #{ori_file}"
								contents = open(ori_file).read.lines
								snippet = contents[line_number-1].strip
								puts "snippet #{snippet}"
								if snippet.start_with?'@' and snippet.split(/[\s=.]/).length == 1
									cur_instr.setLN(line_number, class_name)
									puts "set 1"
								else
									cur_instr.setLN(line_number+1, class_name)
									puts "set 2"
								end
							else
								cur_instr.setLN(line_number+1, class_name)
								if cur_instr.instance_of?Call_instr
									if !cur_instr.ln[4].include?cur_instr.getFuncname
										ori_file = item.gsub('.log','.rb').gsub('dataflow','')
										puts "item file name : #{ori_file}"
										contents = open(ori_file).read.lines
										snippet = contents[line_number+1].strip
										if snippet.include?cur_instr.getFuncname
											line_number += 1
											cur_instr.setLN(line_number+1, class_name)
										end
									end
