...
def read_schema(app_dir)
	...
	file.each_line do |line|
		if line.include?("create_table")
			...
			class_name = @cur_class.getName if @cur_class
			if $const and !$const.include?class_name
				#puts "not read #{class_name} #{tbl_name}"
				next
			end
			# if $known_tables and !$known_tables.include?tbl_name
			# 	@cur_class = nil
			# 	puts "don't read #{tbl_name}"
			# 	#next
			# else
			# 	puts "do read #{tbl_name}"
			# end
