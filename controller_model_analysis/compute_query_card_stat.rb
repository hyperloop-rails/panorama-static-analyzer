$query_return_limited_recs=['any?','blank?','pluck','lock','none','limit','uniq','readonly','new_record','new_record?','exits?','many?','first','second','first!','second!','last','first_or_initialize','find_by_id', "paginate"]
	...
	$node_list.each do |n|
		if n.isReadQuery?
			...
			if @card_limited == false 
				t = n.getInstr.getCallerType
				t_class = $class_map[t]
				f_name = n.getInstr.getFuncname
				puts "t #{t}"
				if $key_words.has_key?(f_name) or (t_class and t_class.getScope.include?f_name)
			...
			if @card_limited
				n.setCardLimited
				puts "n.getInstr.card_limited #{n.getInstr.toString}"
