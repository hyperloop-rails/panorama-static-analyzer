...
class Method_class
	...
	attr_accessor :normal_function, :is_self
	def getVars
		@variables
	end
	def getName
...
class Class_class
	...
	def addMethod(meth)
		if $mcs and !$mcs.include?meth
			return
		end
		key = meth.getName 
		if meth.is_self
			key = meth.getName + "_is_self"
		end
		@methods[key] = meth
		meth.setCallerClass(self)
		#puts "Add method, #{meth.getName} #{@methods.length} "
	end
	def getMethod(name, is_self=false)
		key = name
		if is_self
			key = name + "_is_self"
		end
	...
	def findMethodRecursive(name, step = 0, is_self = false)
		key = name
		key = name+ "_is_self" if is_self
		if @methods[key]
			return @methods[key]
		else
			...
			while $class_map[temp_class_name] != nil and @step < 10
				@step += 1
				f = $class_map[temp_class_name].findMethodRecursive(name, step+1, is_self)
				if f != nil
					return f
				else
			...
			@include_module.each do |inc|
				if $class_map[inc] == nil or (isActiveRecord(@name) and inc.include?("Controller"))
					...
					f = $class_map[inc].findMethodRecursive(name, @step+1, is_self)
					if f
						return f
					end
				end
			...
			if cname = search_distinct_func_name(name)
				if isActiveRecord(@name) and cname.include?("Controller")
					...
					return $class_map[cname].getMethod(name, is_self)
				end
			end
			return nil
		end
