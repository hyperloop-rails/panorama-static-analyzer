...
def traceback_data_dep(cur_node, stop_at_query=false)
	...
	#puts "Start tracebackward #{cur_node.getInstr.toString}"
	while @node_list.length > 0 do
		node = @node_list.pop
		#puts "pop node #{node.getInstr.toString} #{node.getBackwardEdges.length}"
			...
			if e.getFromNode != nil and e.getToNode != nil
				...
				if e.getFromNode.isReadQuery?
					if @dep_array.include?(e.getFromNode)
						...
						#puts "e1: #{e.getFromNode.getInstr.toString}"
					end
				else
					samefile = e.getFromNode.getInstr.getClassName == cur_node.getInstr.getClassName
					...
					if e.getFromNode.getInstr.instance_of?Return_instr and e.getFromNode.getInstr.getClassName == cur_node.getInstr.getClassName and e.getFromNode.getInstr.getMethodName == cur_node.getInstr.getMethodName
						#puts "e3: #{e.getFromNode.getInstr.toString}"
					elsif (samefile and e.getFromNode.getIndex < cur_node.getIndex) or !samefile
					#elsep
						...
						if @dep_array.include?(e.getFromNode)
							...
							@dep_array.push(e.getFromNode)
							@node_list.push(e.getFromNode)
							#puts "e2: #{e.getFromNode.getInstr.toString}"
						end
					else
						#puts "e4: #{e.getFromNode.getInstr.toString} f:#{e.getFromNode.getIndex} t:#{cur_node.getIndex}"
					end
				end
...
def traceforward_data_dep(query_node)
	...
	is_normal = query_node.getInstr.getBB.getCFG.getMHandler.normal_function
	puts "query_node.getDataflowEdges #{query_node.getDataflowEdges.length} #{is_normal}"
	if is_normal == false
		query_node.getDataflowEdges.each do |e|
			@node_list.push(e.getToNode)
			puts "@node_list.push(e.getToNode) #{e.getToNode.getInstr.toString}"
		end
	end
	while @node_list.length > 0 do
		node = @node_list.pop
			...
			if toedge and toedge.getToNode
				toedge.getToNode.getDataflowEdges.each do |e1|
					if e1.getToNode.getIndex <= query_node.getIndex or @traversed.include?(e1.getToNode)
						puts "e1.getToNode.getIndex <= query_node.getIndex"
					else
...
def traceback_control_node(cur_node)
	@dep_array = Array.new
	@node_list = Array.new
	@node_list.push(cur_node)
	while @node_list.length > 0 do
