	...
	if astnode.is_a?YARD::Parser::Ruby::AstNode
		if astnode.type.to_s == "class" or astnode.type.to_s == "module"
			...
			if (@has_sub and @include_function) or @has_sub == false
				#puts "@class_name #{@class_name} #{astnode} #{filename}"
				read_each_class(@class_name, astnode, filename)
			else
...
def sourceIgnore(n1)
	...
	func_ignore_list = ["before_save","before_create","ruby_code_from_view", "transaction", "before_filter_do_block","before_validation_do_block"]
...
def getRealLine(filename, l)
	...
	if($c[filename] and $c[filename][l])
		result = $c[filename][l].dup
	end
	#puts "RESULTS: #{result}"
	if result and result[0] and result[0].include?"/ruby_views/"
		real_code = open(result[0]).read.lines[result[1] - 1]
		...
		begin
			result[4] = open(filename).read.lines[l-1].strip
			#puts "FINALRESULTS: #{result}"
		rescue
		end
	end
...
# get the start position of tag files return start and end postion of the tag

def readTagPostion(file, key)
	# create tags with position information
	
	tag_file = file+".tag"
	rindex = file.index("ruby_views")
	puts "rindex #{rindex} #{file[rindex..-1]}"
	real_file = file[rindex..-1].gsub("ruby_views", "views")
	puts real_file
	tag = Tag.new(real_file, key)
	if File.exist?(tag_file)
		contents = open(tag_file).read
		begin
			postion_hash = eval(contents)
			postion = postion_hash[key]
			tag.start_line =  postion[0]
			tag.start_offset =  postion[1]
			tag.end_line =  postion[2]
			tag.end_offset =  postion[3]
			return tag
		rescue
			return nil
		end
...
def isUnboundedLoop(cl)
	qs = traceback_data_dep(cl, false)
	qs.each do |q|
		if(q.instance_of?Dataflow_edge)
			next
		end
		if q and q.getInstr and q.getInstr.instance_of?Call_instr and ['paginate', '[]'].include?q.getInstr.getFuncname
			nt = false
			puts "funcname: #{q.getInstr.getFuncname} #{q.getInstr.toString}"
			return 
		end
...
def read_known_tables(start_class, start_function)
	puts "read_known_tables"
	know_table_files = $app_dir + "/known_tables"
	puts "know_table_files #{know_table_files}"
	if File.exists?know_table_files
		puts "exists"
		open(know_table_files).each_line do |line|
			items = line.split("\t")
			c_a = items[0]
			tables = items[1]
			puts "#{start_class},#{start_function}"
			puts "c_a #{c_a}"
			if c_a == "#{start_class},#{start_function}"
				$const = tables.strip.split(",")
				$const << start_class
				puts "known_tables #{$known_tables}"
			end
		end
	else
		return
