...
class Instruction
	...
	attr_accessor :defv_var, :args, :inodes, :inode, :symbols, :in_while_loop, :in_view, :ln, :tag_node
	def addSymbol(s)
		@symbols.push(s)
	end
	def getArgs
	...
	def setLN(ln, class_name = nil)
		@line_number = ln
		#puts "ln: #{ln}"
		#puts "setLN class_name #{class_name}"
		filename = $class_map[class_name].filename
		if filename && ln
			@ln = getRealLine(filename, ln) 
			#puts "@LN: #{filename} #{@ln}"
		end
	end
	def getLN
	...
	def toString4
		meth = self.getBB.getCFG.getMHandler
		s = "#{meth.getCallerClass.getName}.#{meth.getName}" 
		return s
	end 
...
class Call_instr < Instruction
	...
	def getTableName
		if $app_dir.include?'tracks' and @funcname == 'current_user'
			return 'users'
		end
	...
	def getQueryType
		...
		if isActiveRecord(t)
			...
			if qtype
				...
				#foreign key relationship: has_many, has_one,...
				tbl_name = self.getCallerType
				#puts "@caller.@func_name #{@caller}.#{@funcname}"
				...
				if isActiveRecord(tbl_name)
					...
					if self.isClassField and @funcname != "user"
						#TODO: check return type, if not know or polomorphism...
						#puts "@funcname is: #{@funcname}"
						@relationship_name = $class_map[tbl_name].searchAssocForRelation(@funcname)
						#puts "relations_name #{@relationship_name}"
			...
			#puts "$class_map[t] #{$class_map[t].getScope}"
			scope_class = t
			while(true)
				if $class_map[scope_class]
					puts "$class_map[scope_class].getScope #{$class_map[scope_class].getScope} #{@funcname}"
					if $class_map[scope_class].getScope.include?@funcname
						return "SELECT"
					end
					scope_class = $class_map[scope_class].getUpperClass
				else
					break
				end
			end
		...
		#puts "@call_handler.getQueryType #{@call_handler}"
...
class AttrAssign_instr < Call_instr
	...
	def toCode
		super
		if getDeps.length > 0
			s = "#{@caller}.#{@funcname} = #{getDeps[0].getVname}" 
		end
		return s
	end
end
