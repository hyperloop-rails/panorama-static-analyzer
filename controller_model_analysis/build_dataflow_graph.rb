...
def handle_single_call_node2(start_class, start_function, instr, level)
		puts "instr.getCallerType :#{start_class}.#{start_function}"
		...
		if instr.isQuery
			...
			if instr_trigger_save?(instr) or instr_trigger_create?(instr)
						...
						if $call_stack_trace.include?(temp_name) or check_nonrepeat(caller_class, action)==false
							...
							if cur_cfg != nil
								...
								prev_defself.each do |p|
									...
									puts "3.5: from : #{p.getInstr.toString} to #{receive_node.getInstr.toString}"
				...
				$def_self_nodes[temp_node].each do |rt|
					...
					puts "7: from : #{rt.getInstr.toString} to #{temp_node.getInstr.toString}"
			...
			temp_name = "#{caller_class}.#{instr.getFuncname} #{instr.toString} #{instr.class.name} #{instr.getCaller}"
			puts "temp_name #{temp_name}"
			is_self = false
			if $class_map[instr.getCaller]
				is_self = true
				puts "is_self set to be true"
			end
			...
			if $call_stack_trace.include?(temp_name) or check_nonrepeat(caller_class, instr.getFuncname)==false
				...
				if instr.getFuncname == "super" and $class_map[caller_class].getUpperClass
					cur_cfg = trace_query_flow($class_map[caller_class].getUpperClass, start_function, "", "", level+1, is_self)
					...
					cur_cfg = trace_query_flow(caller_class, instr.getFuncname, "", "", level+1, is_self)
				end
				$call_stack_trace.pop
				...
				if cur_cfg != nil
					cur_cfg.getExplicitReturn.each do |rt|
						...
						rt.getInstr.getINode.addDataflowEdge(edge)
						puts "returnv from #{rt.getInstr.toString} to #{temp_node.getInstr.toString}"
						$dataflow_edges[dataflow_edge_name] = edge
					end
...
def handle_single_instr2(start_class, start_function, class_handler, function_handler, instr, level, return_list)
	puts "handle_single_instr2 #{instr.toString} #{instr.class.name} #{instr.isQuery}"
...
#	puts "\tInstr: ##{node.getIndex} #{node.getInstr.toString} "
	...
	if instr.is_a?Call_instr
		...
		#puts "start_class, start_function #{start_class} #{start_function} #{instr.getBB.getCFG.getMHandler.is_self}"
	...
	if instr.is_a?ReceiveArg_instr
		if $general_call_stack.length > 0
			...
			if dep_inode != nil
				...
				puts "4: from : #{dep_inode.getInstr.toString} to #{node.getInstr.toString}"
...
def handle_single_cfg2(start_class, start_function, class_handler, function_handler, cfg, level)
	...
	if $funccall_stack.length > 0 and fromIns != nil
		if isValidationFunc(function_handler.getName)
			...
			# puts "5: from : #{dep.getInstr.toString} to #{fromIns.toString}"
...
def trace_query_flow(start_class, start_function, params, returnv, level,is_self=false)
	...
	puts "returnv #{$cur_class}"
	if class_handler != nil
		function_handler = class_handler.findMethodRecursive(start_function, 0, is_self)
		puts "function_handler #{start_function} #{function_handler} #{function_handler.is_self}" if function_handler
	else
